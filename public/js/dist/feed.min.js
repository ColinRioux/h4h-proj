...
function buildFeed(i){$.getJSON(window.location.href+"requests/",function(e){for(var a=e.count,t=[],s=1;s<=a;s++)$.ajax({url:window.location.href+"requests/"+s,dataType:"json",async:!1,success:function(i){t.push(i)}});$(".searchfield").html("");var l=$("#filter").val();for(var n in $(i).html(""),t)if(t[n].tags.indexOf(l)>-1||null==l||""==l){var d=t[n];if($(i).append('<div class="box '+d.id+'" style="width:50%;"><h1 class="title is-4">'+d.title+'</h1><ul class="bxlc"><li><i class="fas fa-users"></i> Size:'+d.size+'</li><li><i class="fas fa-map-marker"></i> Location: '+d.location+'</li></ul><br/><p style="padding-top:2%">'+d.description+'</p><br/><ul class="tgclist"></ul><br/><a href="/job" class="button is-info">Inspect</a></div>'),"string"==typeof d.category){var c=[];c.push(d.category),d.category=c}for(var o in d.category)$(i).find("."+d.id).find(".tgclist").append('<li><div class="jobCategory">'+d.category[o]+"</div></li>");for(var r in $(i).find("."+d.id).find(".tgclist").append('<li style="padding-left:30px;"><i class="fas fa-tags"></i></li>'),d.tags)$(i).find("."+d.id).find(".tgclist").append('<li><div class="jobTag">'+d.tags[r]+"</div></li>")}})}function expandCard(i){window.location.replace("/job")}$=require("jquery"),$(document).ready(function(){buildFeed("#feed","t"),$("#btnfilter").click(function(){buildFeed("#feed")}),$("#refreshJobs").click(function(){buildFeed("#feed")})});
